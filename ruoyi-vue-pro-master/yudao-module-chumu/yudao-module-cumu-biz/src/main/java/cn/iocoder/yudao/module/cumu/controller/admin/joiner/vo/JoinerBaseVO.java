...
/**
 * 畜牧进场 Base VO，提供给添加、修改、详细的子 VO 使用
 * 如果子 VO 存在差异的字段，请不要添加到这里，影响 Swagger 文档生成
 */
@Data
public class JoinerBaseVO {

    @Schema(description = "用户id", requiredMode = Schema.RequiredMode.REQUIRED, example = "17836")
    @NotNull(message = "用户id不能为空")
    private Long customId;

    @Schema(description = "车牌号", requiredMode = Schema.RequiredMode.REQUIRED, example = "12003")
    @NotNull(message = "车牌号不能为空")
    private String carId;

    @Schema(description = "畜牧类别", requiredMode = Schema.RequiredMode.REQUIRED, example = "1")

    private Integer type;

    @Schema(description = "畜牧数量", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "畜牧数量不能为空")
